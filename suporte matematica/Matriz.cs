...
namespace ProjetoMatrizesNaoQuadraticas.bibliotecasMatematicas
{
    public class Matriz
    {
        ...
        public static Matriz Cofactor(Matriz m, int linha, int coluna)
        {
            Matriz mt_cofator = new Matriz(m.lines, m.columns);
            List<List<double>> lst_matrix = ToListMatrix(m);
            lst_matrix.RemoveAt(linha);


            for (int lin = 0; lin < lst_matrix.Count; lin++)
                lst_matrix[lin].RemoveAt(coluna);
            return ToArrayMatrix(lst_matrix);
        ...
        public double Determinante()
        {
            ...
            for (int col = 0; col < this.columns; col++)
            {
                ...
                if ((0 + col) % 2 == 0)
                    ...
                    signal = -1;
                Matriz cof = Cofactor(this, 0, col);

                det += signal * this.data[0, col] * cof.Determinante();
            }
            return det;
        }

        public Matriz GetMatrixCofactors()
        {
            Matriz mt_cofator = new Matriz(this.lines, this.columns);

            for (int linha = 0; linha < this.lines; linha++)
                for (int coluna = 0; coluna < this.columns; coluna++)
                    mt_cofator.data[linha, coluna] = Cofactor(this, linha, coluna).Determinante();

            return mt_cofator;
        }

        public static Matriz Transpose(Matriz mt_to_transpose)
        {
            if (mt_to_transpose.lines != mt_to_transpose.columns)
                return null;
            else
            {
                Matriz mt_transposed = new Matriz(mt_to_transpose.columns, mt_to_transpose.lines);


                for (int lin = 0; lin < mt_to_transpose.lines; lin++)
                    for (int col = 0; col < mt_to_transpose.columns; col++) 
                        mt_transposed.data[lin, col] = mt_to_transpose.data[col, lin];

            
                return mt_transposed;
            }
        }
        ...
        public static Matriz Add(Matriz m1, Matriz m2)
        {
            ...
            Matriz mt_added = new Matriz(m1.lines, m1.columns);
            for (int lin = 0; lin < m1.lines; lin++)
                for (int col = 0; col < m1.columns; col++)
                    mt_added.data[lin, col] = m1.data[lin, col] + m2.data[lin, col];
        ...
        {
            if ((m1 == null) || (m2 == null) || ((m1.lines != m2.columns) && (m1.columns != m2.lines)))
                return null;
            else
            {
                Matriz m_mul = new Matriz(m1.lines, m2.columns);
                for (int lin = 0; lin < m1.lines; lin++)
                    for (int col = 0; col < m2.columns; col++)
                        for (int k = 0; k < m1.columns; k++)
                            m_mul.data[lin, col] += m1.data[lin, k] * m2.data[k, col];
        ...
        public static Matriz Mul(double number,Matriz m1)
        {
            ...
            {
                Matriz m_mul = new Matriz(m1.lines, m1.columns);
                for (int lin = 0; lin < m1.lines; lin++)
                    for (int col = 0; col < m1.columns; col++)
                        m_mul.data[lin, col] = number * m1.data[lin, col];
        ...
        public static  void PrintMatrix(string caption, Matriz mt_to_print)
        {
            ...
            System.Console.WriteLine("determinante: " + mt_to_print.Determinante());
            System.Console.WriteLine();
            System.Console.WriteLine();
        }
        ...
        public class Testes : SuiteClasseTestes
        {
            ...
            {
                Matriz mt_teste = new Matriz(3, 3);
                GetRandomMatriz(ref mt_teste, 1005);
                mt_teste.data[0, 0] = 2.0;
                mt_teste.data[1, 1] = 5.0;
                mt_teste.data[2, 2] = 7.0;


                Matriz cofator_00 = Matriz.Cofactor(mt_teste, 0, 0);
                Matriz cofator_11 = Matriz.Cofactor(mt_teste, 1, 1);

                Matriz.PrintMatrix("matriz base: ", mt_teste);
                Matriz.PrintMatrix("cofator[0,0]: ", cofator_00);
                Matriz.PrintMatrix("cofator[1,1]: ", cofator_11);
            ...
            {
                MyTransformacaoIsometrica.precisaoDigitos = 25;

                Matriz mt_teste = new Matriz(3, 3);
                GetRandomMatriz(ref mt_teste, 1503);
                Matrix<double> mt_teste_lib = this.MatrizToMatrixDouble(mt_teste);


                Matriz mt_teste_inversa = Inversa(mt_teste);

                System.Console.WriteLine("determinante calculado:" + mt_teste.Determinante());
                System.Console.WriteLine("determinante lib: " + mt_teste_lib.Determinant());


                Matriz.PrintMatrix("matriz original: ", mt_teste);
                Matriz.PrintMatrix("matriz inversa: ", mt_teste_inversa);
                System.Console.WriteLine("matriz inversa lib: "+ mt_teste_lib.Inverse());

                assercao.IsTrue(Math.Abs(mt_teste_inversa.data[0, 0] - mt_teste_lib.Inverse()[0, 0]) < 0.001);
                System.Console.ReadLine();
            }
            ...
            public void Teste_Determinante(AssercaoSuiteClasse assercao)
            {
                ...
                Matriz mt_teste = new Matriz(3, 3);
                GetRandomMatriz(ref mt_teste, 10057);

                Matrix<double> mt_teste_lib = MatrizToMatrixDouble(mt_teste);

                double determinateTestes = mt_teste.Determinante();
